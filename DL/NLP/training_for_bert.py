...
def bert_main(seed, cyp_name, model_name, bayes_choice=True):
    ...
    train_data = pd.read_csv("data/cyp{}_train_MACCS.csv".format(cyp_name)).loc[:, ["Smiles", "Y"]]
    valid_data = pd.read_csv("data/cyp{}_valid_MACCS.csv".format(cyp_name)).loc[:, ["Smiles", "Y"]]
    test_data = pd.read_csv("data/cyp{}_test_MACCS.csv".format(cyp_name)).loc[:, ["Smiles", "Y"]]
    columns = ["Smiles", "Y"]
    ...
    torch.save((train_dataloader, valid_dataloader), "validation_dataset/Bert_%s.pt" % cyp_name)
    ...
    for epoch in range(b_epoch):
        ...
        if va_r[0] >= 0.85:
            torch.save(model.state_dict(), "model_save/MTL_BERT/{}/epoch_{}.pth".format(cyp_name, epoch))
