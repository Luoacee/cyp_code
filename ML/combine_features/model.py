    ...
    for sub in isoforms:
        ...
        for mn in model_name:
            for fdx, fi in enumerate(finger_list):
                for ddx, di in enumerate(dis_list):
                    print('====== Processing: Model:{}   isoforms:{}   fingerprint:{} ====== '.format(mn, sub, fi))

                    train_path = 'datasets/Dataset_In_{}/{}_train_{}.csv'.format(fi, sub, fi)
                    tr_desc_path = 'datasets/Dataset_In_MOLD2/{}_train_{}.csv'.format(sub, ddx)
                    test_path = 'datasets/Dataset_In_{}/{}_test_{}.csv'.format(fi, sub, fi)
                    te_desc_path = 'datasets/Dataset_In_MOLD2/{}_test_{}.csv'.format(sub, ddx)
                    _des_name = "{}_{}".format(fi, ddx)
                    d = DataProcessing(p_name=sub, d_name=_des_name)
                    train_x, train_y = d(train_path, tr_desc_path)
                    test_x, test_y = d.test_set(test_path, te_desc_path)
                    ...
                    if mdx == 0:
                        model_record = pd.DataFrame([[sub, mn, fi, best_model_info]], columns=['isoforms',
                                                                                               'model',
                                                                                               'char',
                                                                                               'params'])
                        mdx = 1
